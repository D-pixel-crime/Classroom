...
const ManageStudents = ({ setIsOpen }: paramProps) => {
  const [allStudents, setAllStudents] = useState<teacherProps[]>([]);
    ...
    const fetchDetails = async () => {
      try {
        const { data } = await axios.get(
          `${import.meta.env.VITE_CLASSROOM_BACKEND_URI}/get/all-students`,
  ...
  const updateStudent = async () => {
    ...
    try {
      await axios.patch(
        `${import.meta.env.VITE_CLASSROOM_BACKEND_URI}/patch/update-student/${
          selectedStudent._id
        }`,
        { email: selectedStudent.email, password },
        { withCredentials: true }
      );
  ...
  const deleteStudent = async () => {
    try {
      await axios.delete(
        `${import.meta.env.VITE_CLASSROOM_BACKEND_URI}/delete/delete-student/${
          selectedStudent._id
        }`,
        { withCredentials: true }
      );
  ...
  return (
    <div className="fixed flex-center flex-col top-0 left-0 w-screen h-screen bg-black z-50 bg-opacity-70 text-white">
      <div className="border-2 border-slate-600 rounded-md p-4 bg-slate-800 flex-col flex gap-5">
        ...
        <h1 className="border-b-2 pb-2 border-b-slate-600">Manage Students</h1>
          ...
          >
            ...
            {allStudents &&
              allStudents.map((eachStudent) => (
                <option key={eachStudent._id} value={eachStudent._id}>
                  {eachStudent.email}
                </option>
              ))}
          </select>
        </div>
        {selectedStudent.email.length > 0 && (
